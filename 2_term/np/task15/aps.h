...
istream& operator >>(istream& is, Aps& aps)
{
    ...
    for (int i = 0; i < aps.n; i++)
    {
        ...
        int pos = 0, pos1 = str.find(' ');
        if (pos1 - pos == 1 && (str[0] != 'c' || str[0] != 'l' || str[0] != 'i'))
            type = str[0];
        else
        {
            cerr << "Wrong code of " << q << " element.\n";
            Consumer** s = new Consumer*[aps.n-1];
            for (int j = 0; j < i; j++)
                s[j] = aps.station[j];
            for (int j = i + 1; j < aps.n; j++)
                s[j - 1] = aps.station[j];
        ...
        try
        {
            consumerNumber = stoi(str.substr(pos, pos1 - pos));
        }
        catch(const exception& e)
        {
            cerr << "Wrong consumer number in " << q << " element.\n";
            Consumer** s = new Consumer*[aps.n-1];
            for (int j = 0; j < i; j++)
                s[j] = aps.station[j];
            for (int j = i + 1; j < aps.n; j++)
        ...
        try
        {
            mobileNumber = stol(str.substr(pos, pos1 - pos));
        }
        catch(const exception& e)
        {
            cerr << "Wrong mobile number in " << q << " element.\n";
            Consumer** s = new Consumer*[aps.n-1];
            for (int j = 0; j < i; j++)
                s[j] = aps.station[j];
            for (int j = i + 1; j < aps.n; j++)
        ...
        try
        {
            debt = stoi(str.substr(pos, pos1 - pos));
        }
        catch(const exception& e)
        {
            cerr << "Wrong debt in " << q << " element.\n";
            Consumer** s = new Consumer*[aps.n-1];
            for (int j = 0; j < i; j++)
                s[j] = aps.station[j];
            for (int j = i + 1; j < aps.n; j++)
